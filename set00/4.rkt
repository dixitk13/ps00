...
(define (tip AmountOfTheBill PercentageOfTip)
 (cond 
   ...
   [(negative? PercentageOfTip) "Percentage of Bill cannot be negative"]
   [else (* AmountOfTheBill PercentageOfTip)]))

(check-expect (tip 10 0.15) 1.5 )
(check-expect (tip 20 0.17) 3.4)
(check-expect (tip -20 0.17) "Amount of Bill cannot be negative" )
(check-expect (tip 20 -0.17) "Percentage of Bill cannot be negative")
